...
from src.repository import auth as repository_users, contacts
...
async def confirmed_email(token: str, db: Session = Depends(get_db)):
    ...
    await contacts.confirmed_email(email, db)
...
async def reset(token: str, password: RequestPassword, db: Session = Depends(get_db)):
    ...
    await contacts.update_password(email, hash_password, db)
